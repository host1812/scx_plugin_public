...
using namespace SCXCoreLib::Xml;
...
namespace SCXSystemLib
{
                        ...
                        cServiceBindingManagerName == codeprop)
                    {
                        ...
                        for (size_t attrNodeIdx = 0; !(foundname && foundurl) && attrNodeIdx < attrNodes.size(); ++attrNodeIdx)
                        {
                                ...
                                attrNodes[attrNodeIdx]->GetAttributeValue(cNameAttributeName, nameprop))
                            {
                                ...
                                {
                                    foundname = true;
                                    serverName = attrNodes[attrNodeIdx]->GetContent();
                                }
                                else if (cStoreUrlName == nameprop)
                                {
                                    foundurl = true;
                                    storeUrl = StrFromUTF8(attrNodes[attrNodeIdx]->GetContent());
                                }
                            }
                        }
    ...
    */
    bool GetPortsBinding (XElementList paramNodes, wstring & portsBinding)
    {
        ...
        {
            string content = paramNodes[0]->GetContent();
                ...
                for (size_t idx = 0; !(foundset && foundbase) && idx < beanNodes.size(); ++idx)
                {
                        ...
                        cServiceBindingSetName == classprop)
                    {
                        ...
                        if (beanNodes[idx]->GetChild(cConstructorNodeName, constrNode))
                        {
                            ...
                            {
                                TryReadInteger(portOffset, foundset, StrFromUTF8(paramNodes[2]->GetContent()), L"Failed to parse port offset");
                            }
                        }
                    }
                    ...
                    for (size_t idx = 0; !(foundHttp && foundHttps) && idx < setBeanNodes.size(); ++idx)
                    {
                            ...
                            propNodes[0]->GetContent() == cWebServerName)
                        {
                                ...
                                cPortName == nameprop)
                            {
                                ...
                                {
                                    TryReadInteger(baseHttpPort, foundHttp, StrFromUTF8(propNodes[2]->GetContent()), L"Failed to parse HTTP port");
                                }
                                else if (propNodes[1]->GetContent() == cHttpsConnectorName)
                                {
                                    TryReadInteger(baseHttpsPort, foundHttps, StrFromUTF8(propNodes[2]->GetContent()), L"Failed to parse HTTPS port");
                                }
                            }
                                     ...
                                     cPortName == nameprop)
                            {
                                TryReadInteger(baseHttpPort, foundHttp, StrFromUTF8(propNodes[1]->GetContent()), L"Failed to parse HTTP port");
                            }
                        }
                    }
