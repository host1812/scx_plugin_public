...
*/
    void SCXCondition::SetSleep(scxulong milliseconds)
    {
        ...
        {
            throw SCXInternalErrorException(L"Can only call SetSleep() once", SCXSRCLOCATION);
        }
    }
...
*/
    enum SCXCondition::eConditionResult SCXCondition::Wait()
    {
        ...
        if (err != 0)
        {
            if (ETIMEDOUT == err)
            {
                // We got a timeout, so recompute next time
                m_bRecomputeSleep = true;
