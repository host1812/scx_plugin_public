...
#error "Platform not supported"
    ...
    */
    SCXFilePath SCXFileSystem::CreateFullPath(const SCXFilePath& path) {
        ...
        std::vector<std::wstring> filePathTokens;
        ...
        std::vector<std::wstring> resolvedFilePathTokens;
        for (std::vector<std::wstring>::const_iterator token = filePathTokens.begin();
        ...
        for (std::vector<std::wstring>::const_iterator token = resolvedFilePathTokens.begin();
...
#error
    ...
    */
    ...
    /**
       ...
       \returns    Textual representation
    */
    std::wstring SCXFileSystem::AsText(const SCXFileSystem::Attributes& attributes) {
        wostringstream buf;
        bool firstItem = true;
    ...
    */
    void SCXFileSystem::Stat(const SCXFilePath& path, SCXStatStruct* pStat)
    {
...
#if defined(WIN32)
        std::wstring mangledPath = path.Get();
        ...
        if (mangledPath.size() != std::wstring(L"C:\\").size() ||
            mangledPath[1] != L':' ||
            mangledPath[2] != L'\\')
        {
...
#error
        ...
        if (failure) {
            ...
            case ENOTDIR:
                throw SCXFilePathNotFoundException(path, SCXSRCLOCATION);
            default:
                std::wstring problem(L"Failed to set attributes for " + path.Get());
...
#error
        ...
        if (failure) {
            if (errno == EINVAL) {
                ...
                std::wstring problem(L"Failed to move " + oldPath.Get());
                throw SCXInternalErrorException(UnexpectedErrno(problem, errno), SCXSRCLOCATION);
            }
        }
