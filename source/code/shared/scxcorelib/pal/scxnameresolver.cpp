...
namespace SCXCoreLib 
{
    ...
    */
    ...
    /**
       ...
       \return A wstring containing a serialized version of this class.
    */
    wstring NameResolverInternal::DumpString() const
    {
    ...
    */
    wstring NameResolverInternal::DumpSourceString(eNameResolverSource e) const
    {
        switch(e) {
    ...
    */
    wstring NameResolverInternal::GetHostname() const
    {
        return m_hostname;
    }
    ...
    */
    wstring NameResolverInternal::GetDomainname() const
    {
        return m_domainname;
    }
    ...
    */
    wstring NameResolverInternal::GetHostDomainname() const
    {
...
/*----------------------------------------------------------------------------*/
    ...
    */
    void NameResolverInternal::GetHostVia_uname()
    {
        ...
        if('\0' != buf.domainname[0]) 
        {
            ...
            m_domainname = StrFromMultibyte(buf.domainname);
        }
#endif
    ...
    */
    void NameResolverInternal::GetHostVia_gethostname()
    {
        ...
        if(0 == rc)
        {
            m_hostname = StrFromMultibyte(&buf[0]);
    ...
    */
    std::wstring NameResolverInternal::GetHostByName(const std::wstring & name) const
    {
        ...
        struct hostent * ph = 0;
        int herrno;
        int rc = gethostbyname_r(StrToMultibyte(name).c_str(), &h, &abuf[0], bufsize,
                                 &ph, &herrno);
        if((0 == rc) && (0 != ph)) 
        {
            return StrFromMultibyte(h.h_name);
        }
        else if(ERANGE == rc)
        {
    ...
    */
    std::wstring NameResolverInternal::GetHostByName(const std::wstring & name) const
    {
        ...
        struct hostent h;
        int herrno;
        struct hostent *ph = gethostbyname_r(StrToMultibyte(name).c_str(),
                                             &h,
                                             &abuf[0],
                                             bufsize,
                                             &herrno);
    ...
    /**
       ...
       thread support.
    */
    wstring NameResolverInternal::GetHostByName(const wstring & name) const
    {
        struct hostent *ph = gethostbyname(StrToMultibyte(name).c_str());
        if(0 != ph)
        {
            return StrFromMultibyte(ph->h_name);
        }
        return L"";
    }
...
#  error NameResolverInternal::GetHostByName() was not implemented for this platform.
    ...
    */
    void NameResolverInternal::GetHostVia_ResolvConf(const std::string & name)
    {
        ...
        while( is && getline(is, line) )
        {
            if(0 == line.find(L"domain"))
            {
                // Strip leading whitespace.
                string::size_type pos = 6;
                string::size_type size = 0;
                while(isspace(line[pos+size]) && ((pos+size) < line.size()))
                {
                    ++size;
                }
