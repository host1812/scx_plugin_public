...
/**
    ...
    a named lock but rather an anonymous lock.
    
*/
    SCXThreadLock::SCXThreadLock(const std::wstring& nameOfLock, bool aquire /*= true*/)
    {
        m_lock = SCXThreadLockFactory::GetInstance().GetLock(nameOfLock);
        if (aquire)
            Lock();
    }
