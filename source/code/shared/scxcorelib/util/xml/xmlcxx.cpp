...
const char* XMLElement::GetAttributeValue(size_t index) const
{
    if (index < _rep->elem.attrsSize)
    {
        const char *val = _rep->elem.attrs[index].value;
...
const char* XMLElement::FindAttributeValue(const char* name) const
{
    for (size_t i = 0; i < _rep->elem.attrsSize; i++)
    {
        ...
        {
            const char *val = _rep->elem.attrs[i].value;
...
public:
    ...
    XMLReaderRep()
    {
        XML_Init(&xml);
        text.clear();
        text.insert(text.end(), NULLBYTE);
    }

    void SetText(const char* text_)
    {
        int len = (int)strlen(text_);
        ...
        if (len > 0)
        {
            ...
            if (strlen(text_) > 0)
            {
                text.insert(text.end(), text_, text_ + strlen(text_) + 1);
            }
        }
...
static void _PutRaw(XMLWriterRep* rep, const char* str)
{
    ...
    if (len)
        Buf_App(&rep->buf, str, (MI_Uint32)len);
}
...
static void _PutChars(XMLWriterRep* _rep, const char* str)
{
    ...
    while (*p)
    {
        Buf_App(&_rep->buf, _encode[*p].str, (MI_Uint32)_encode[*p].len);
        p++;
    }
}
...
{
    return string(reinterpret_cast<const char*>(_rep->buf.data), _rep->buf.size);
}
