...
namespace {
    ...
    public:
        ...
        ProcLister() : ent(0) {
            d = opendir("/proc/");
            if (!d) { throw SCXErrnoException(L"opendir", errno, SCXSRCLOCATION); }
        };
        ...
        bool nextProc() {
            ...
            if (errno != 0) { throw SCXErrnoException(L"readdir", errno, SCXSRCLOCATION); }
            return false;
        }
    ...
    private:
        ...
        bool readburst() {
            ...
            if (m_burst < 0) { throw SCXErrnoException(L"pstat_getproc", errno, SCXSRCLOCATION); }
...
namespace SCXSystemLib
{
    ...
    */
    ...
    /**
       ...
       to lock that thread first. See ProcessEnumeration::GetLockHandle().
     */
    std::vector<SCXCoreLib::SCXHandle<ProcessInstance> > ProcessEnumeration::Find(const std::wstring& name)
    {
        ProcMap::iterator pi;
    ...
    */
    bool ProcessEnumeration::SendSignalByName(const std::wstring& name, int sig)
    {
